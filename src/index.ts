...
async function main() {
  ...
  //@audit pay attention, getUniswapMarketsByToken this function is static function....
  //@audit to get all valid market pair for every token
  //@audit search all pairs in factory_address
  const markets = await UniswappyV2EthPair.getUniswapMarketsByToken(provider, FACTORY_ADDRESSES);
  
  provider.on('block', async (blockNumber) => {
    // update all reserves in marketpair
    ...
    // to swap on every best cross market 
